...
# 01-orchestration：オーケストレーションとは何か
...
1. [Compose の限界](#compose-の限界)
2. [複数マシンの管理という課題](#複数マシンの管理という課題)
3. [オーケストレーションとは何か](#オーケストレーションとは何か-1)
4. [オーケストレーションが解決する問題](#オーケストレーションが解決する問題)
5. [Kubernetes の誕生と背景](#kubernetes-の誕生と背景)
6. [次のトピックへ](#次のトピックへ)
7. [用語集](#用語集)
8. [参考資料](#参考資料)
...
| 限界              | 具体的な問題                                     |
| ----------------- | ------------------------------------------------ |
| 単一マシン        | 1 台のマシンの性能が上限。マシン故障ですべて停止 |
| 再起動のみ        | 同じマシンでの再起動だけ。マシン故障時は復旧不能 |
| 手動スケーリング  | 人間が手動で増減。同じマシン上に限定             |
...
次のトピック [02-architecture](./02-architecture.md) では、オーケストレーションの<strong>アーキテクチャ</strong>を学びます
...
| 再起動ポリシー（Restart Policy）                   | コンテナが停止した際の再起動方法を定義する設定。`always`、`on-failure` などがある                                                   |
...
| 命令的（Imperative）                               | 「何をするか」の具体的な操作手順を指示する方法。「コンテナを 1 つ起動しろ」のような操作                                             |
| 宣言的（Declarative）                              | 「どうあるべきか」の最終状態を宣言する方法。「コンテナが 3 つ動いている状態にしろ」のような宣言                                     |
| あるべき状態（Desired State）                      | システムが維持すべき目標の状態。管理者が宣言し、オーケストレーションが維持する                                                      |
| 調整ループ（Reconciliation Loop）                  | あるべき状態と実際の状態を継続的に比較し、差分を自動で修正するメカニズム                                                            |
| Borg                                               | Google が社内で運用していた大規模クラスタ管理システム。Kubernetes の設計に影響を与えた                                              |
| Kubernetes（K8s）                                  | Google の Borg の経験を活かして設計された、オープンソースのコンテナオーケストレーション基盤。名前はギリシャ語で「操舵手」を意味する |
| CNCF（Cloud Native Computing Foundation）          | Kubernetes を含むクラウドネイティブ技術のオープンソースプロジェクトを管理する中立的な組織                                           |
| レプリカ（Replica）                                | 同じコンテナの複製。負荷分散や可用性のために複数のレプリカを実行する                                                                |
| コントロールプレーン（Control Plane）              | クラスタ全体を管理する中枢。スケジューリング、状態管理、API 提供などを担当する                                                      |
