...
# [appendix：スプリットブレイン問題](#split-brain) {#split-brain}

## [はじめに](#introduction) {#introduction}

[02-architecture](../../02-architecture/) では、etcd がクラスタのすべての状態を保存する<strong>分散キーバリューストア</strong>であり、<strong>唯一の情報源（Single Source of Truth）</strong>として機能することを学びました

また、[05-self-healing](../../05-self-healing/) では、ノードからのハートビートが途絶えた場合に Node コントローラがノードを NotReady にマークし、Pod を退避させる仕組みを学びました
...
1. [スプリットブレインとは](#what-is-split-brain)
2. [ネットワーク分断のシナリオ](#network-partition-scenarios)
3. [etcd とクォーラム](#etcd-and-quorum)
4. [ノード障害検知の限界](#node-failure-detection-limitation)
5. [二重実行の可能性](#double-execution-possibility)
6. [Lease とフェンシング](#lease-and-fencing)
7. [用語集](#glossary)
8. [参考資料](#references)

---

## [スプリットブレインとは](#what-is-split-brain) {#what-is-split-brain}
...
## [ネットワーク分断のシナリオ](#network-partition-scenarios) {#network-partition-scenarios}
...
### [シナリオ 1：コントロールプレーンとワーカーノード間の分断](#scenario-control-plane-partition) {#scenario-control-plane-partition}
...
### [シナリオ 2：etcd クラスタ内部の分断](#scenario-etcd-partition) {#scenario-etcd-partition}
...
### [「死亡」ではなく「到達不能」](#unreachable-not-dead) {#unreachable-not-dead}
...
## [etcd とクォーラム](#etcd-and-quorum) {#etcd-and-quorum}

スプリットブレインに対する Kubernetes の最も重要な防御は、etcd が内部で使用する<strong>Raft 合意アルゴリズム</strong>のクォーラム（過半数）の仕組みです

[raft-consensus](../raft-consensus/) で学んだ Raft の過半数の原則が、ここで重要な役割を果たします

### [クォーラムの仕組み](#quorum-mechanism) {#quorum-mechanism}

etcd は、書き込みを受け付けるために<strong>過半数（クォーラム）のインスタンスの合意</strong>を必要とします

[raft-consensus](../raft-consensus/) で見た通り、この過半数の条件により、2 つのグループが同時に異なるデータを書き込むことが防がれます

どの 2 つの過半数にも、必ず少なくとも 1 つの共通メンバーが存在するためです

### [分断時の挙動](#partition-behavior) {#partition-behavior}
...
[raft-consensus](../raft-consensus/) で「etcd のインスタンス数に奇数が推奨される」理由を学びましたが、このシナリオはその理由をさらに補強します

偶数ノードでは均等分断時にクラスタ全体が停止するリスクがあるのに対し、奇数ノードでは必ず多数派が存在します

### [クォーラムの限界](#quorum-limitations) {#quorum-limitations}
...
## [ノード障害検知の限界](#node-failure-detection-limitation) {#node-failure-detection-limitation}

[05-self-healing](../../05-self-healing/) で学んだように、Kubernetes はハートビートと Lease の仕組みでノードの正常性を監視しています

ノードからのハートビートが 50 秒途絶えると、Node コントローラはそのノードを NotReady にマークします

しかし、この仕組みには根本的な限界があります

### [「停止」と「到達不能」の区別ができない](#cannot-distinguish-stopped-and-unreachable) {#cannot-distinguish-stopped-and-unreachable}
...
### [分断されたノードの視点](#partitioned-node-perspective) {#partitioned-node-perspective}
...
## [二重実行の可能性](#double-execution-possibility) {#double-execution-possibility}

ノード障害検知の限界から、ネットワーク分断時に<strong>同じアプリケーションの同じ Pod が 2 つのノードで同時に動く「二重実行」</strong>状態が発生する可能性があります

### [二重実行が発生する流れ](#double-execution-flow) {#double-execution-flow}

[05-self-healing](../../05-self-healing/) で学んだノード障害時の Pod 退避の流れを、ネットワーク分断の観点から見直してみましょう
...
### [ステートレスとステートフルでの影響の違い](#stateless-stateful-impact-difference) {#stateless-stateful-impact-difference}
...
## [Lease とフェンシング](#lease-and-fencing) {#lease-and-fencing}

スプリットブレイン問題を完全に解決することは困難ですが、影響を緩和するための手法がいくつか存在します

### [Lease（リース）](#lease) {#lease}

<strong>Lease</strong>は、リソースの利用権に<strong>有効期限を設ける</strong>仕組みです

[05-self-healing](../../05-self-healing/) で学んだ Lease オブジェクトは、ノードの生存確認に使われていました
...
### [フェンシング（Fencing）](#fencing) {#fencing}
...
### [Kubernetes での対応](#kubernetes-countermeasures) {#kubernetes-countermeasures}
...
### [アプリケーション側の設計](#application-side-design) {#application-side-design}
...
| クォーラム（Quorum） | 操作を確定するために必要な最小限のノード数であり、過半数で定義される<br>etcd はクォーラムの合意なしに書き込みを受け付けない |
| Lease（リース） | リソースの利用権に有効期限を設ける仕組み<br>期限内に更新しなければ権限が失効する |
...
| 冪等性（Idempotency） | 同じ操作を何度実行しても結果が変わらない性質<br>二重実行が発生しうる環境での重要な設計原則 |
| NotReady | ノードがハートビートを送信しなくなった場合にマークされる状態<br>Node コントローラがこの状態への変更を行う |
...
- [Node Lease](https://kubernetes.io/docs/concepts/architecture/leases/){:target="\_blank"}
