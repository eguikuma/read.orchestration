...
# [appendix：Raft 合意アルゴリズム](#raft-consensus) {#raft-consensus}
...
1. [なぜ合意が必要か](#why-consensus)
2. [Raft の基本概念](#raft-basic-concepts)
3. [リーダー選出](#leader-election)
4. [ログの複製](#log-replication)
5. [安全性の保証](#safety-guarantee)
6. [etcd における Raft](#raft-in-etcd)
7. [用語集](#glossary)
8. [参考資料](#references)

---

## [なぜ合意が必要か](#why-consensus) {#why-consensus}

[02-architecture](../../02-architecture/) で学んだように、etcd はクラスタの全状態を保存する唯一の情報源です
...
### [データの不一致が起きる理由](#data-inconsistency-reason) {#data-inconsistency-reason}
...
### [不一致がもたらす問題](#inconsistency-problems) {#inconsistency-problems}
...
### [合意が解決すること](#what-consensus-solves) {#what-consensus-solves}
...
## [Raft の基本概念](#raft-basic-concepts) {#raft-basic-concepts}
...
### [3 つの役割](#three-roles) {#three-roles}
...
## [リーダー選出](#leader-election) {#leader-election}
...
### [選出の流れ](#election-flow) {#election-flow}
...
### [選挙タイムアウトのランダム化](#election-timeout-randomization) {#election-timeout-randomization}
...
### [なぜ過半数なのか](#why-majority) {#why-majority}
...
## [安全性の保証](#safety-guarantee) {#safety-guarantee}
...
### [なぜこの性質が成り立つか](#why-this-property-holds) {#why-this-property-holds}
...
## [etcd における Raft](#raft-in-etcd) {#raft-in-etcd}

[02-architecture](../../02-architecture/) で学んだ etcd は、内部で Raft を使用してデータを複数のインスタンス間で複製しています

ここでは、Raft の仕組みが etcd でどのように活用されているかを見ていきます

### [クォーラムとインスタンス数](#quorum-and-instance-count) {#quorum-and-instance-count}
...
### [なぜ奇数が推奨されるか](#why-odd-number-recommended) {#why-odd-number-recommended}
...
### [書き込みのコスト](#write-cost) {#write-cost}
...
### [Single Source of Truth の実現](#single-source-of-truth) {#single-source-of-truth}

[02-architecture](../../02-architecture/) では、etcd がクラスタの<strong>唯一の情報源（Single Source of Truth）</strong>であると学びました
...
これが、[02-architecture](../../02-architecture/) で述べた「etcd は複数インスタンスで動作し、1 つが壊れてもデータが失われない」の背後にある仕組みです
...
- [Raft Visualization](https://raft.github.io/){:target="\_blank"}
