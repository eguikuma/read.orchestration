...
# 03-scheduling：スケジューリング

## はじめに

前のトピック [02-architecture](./02-architecture.md) では、オーケストレーションのアーキテクチャを学びました
...
1. [スケジューリングとは何か](#スケジューリングとは何か)
2. [フィルタリング（候補の絞り込み）](#フィルタリング候補の絞り込み)
3. [スコアリング（最適なノードの選択）](#スコアリング最適なノードの選択)
4. [リソース要求と制限](#リソース要求と制限)
5. [ラベルと nodeSelector](#ラベルと-nodeselector)
6. [Node Affinity（ノードアフィニティ）](#node-affinityノードアフィニティ)
7. [Taint と Toleration](#taint-と-toleration)
8. [Scheduling Framework（拡張可能な設計）](#scheduling-framework拡張可能な設計)
9. [OS スケジューラとの対比](#os-スケジューラとの対比)
10. [次のトピックへ](#次のトピックへ)
11. [用語集](#用語集)
12. [参考資料](#参考資料)
...
### nodeSelector と Node Affinity の比較
...
次のトピック [04-service-discovery](./04-service-discovery.md) では、<strong>サービスディスカバリ</strong>の仕組みを詳しく学びます
